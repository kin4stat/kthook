...
#define EQUALITY_CHECK(x)                                           \
    if (this->x != rhs.x) {               \
        return testing::AssertionFailure() << "this->" << ##x << " != " << "rhs." << ##x; \
    }
...
TEST(KthookSignalTest, BigAggregate) {
    ...
    {
        auto conn = hook.before.scoped_connect([](const auto& hook, BigAggregate& value) -> std::optional<BigAggregate> {
            EXPECT_TRUE(big_args == value);
            return std::nullopt;
        });
        EXPECT_TRUE(big_test_func(big_args) == big_args);
...
TEST(KthookSignalTest, MediumAggregate) {
    ...
    {
        auto conn = hook.before.scoped_connect([](const auto& hook, MediumAggregate& value) -> std::optional<MediumAggregate> {
            EXPECT_TRUE(medium_args == value);
            return std::nullopt;
        });
        EXPECT_TRUE(medium_test_func(medium_args) == medium_args);
...
TEST(KthookSignalTest, SmallAggregate) {
    ...
    {
        auto conn = hook.before.scoped_connect([](const auto& hook, SmallAggregate& value) -> std::optional<SmallAggregate> {
            EXPECT_TRUE(small_args == value);
            return std::nullopt;
            });
        EXPECT_TRUE(small_test_func(small_args) == small_args);
