...
namespace kthook {
	namespace detail {
		...
		struct signal_relay_generator<HookType, Ret, std::tuple<Ts...>> {
			static Ret relay(Ts... args, HookType* this_hook) {
				return signal_relay<HookType, Ret, Ts...>(this_hook, args...);
			}
		};
		...
		inline std::uintptr_t find_prev_free(std::uintptr_t from, std::uintptr_t to, std::uintptr_t granularity) {
			...
			while (from < to) {
				...
				if (mbi.State == MEM_FREE)
					return to;
				if (reinterpret_cast<std::uintptr_t>(mbi.AllocationBase) < granularity)
					break;
			...
			to -= granularity;
			while (from < to) {
				void* alloc = mmap(reinterpret_cast<void*>(to), granularity,
					PROT_EXEC | PROT_READ | PROT_WRITE,
					MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED_NOREPLACE, 0, 0);
				if (reinterpret_cast<std::uintptr_t>(alloc) != 0xffffffffffffffff)
					return to;
				if (to < granularity)
					break;
				to = to - granularity;
		...
		inline std::uintptr_t find_next_free(std::uintptr_t from, std::uintptr_t to, std::uintptr_t granularity) {
			...
			from += granularity;
			while (from <= to) {
				void* alloc = mmap(reinterpret_cast<void*>(from), granularity,
					PROT_EXEC | PROT_READ | PROT_WRITE,
					MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED_NOREPLACE, 0, 0);
				if (reinterpret_cast<std::uintptr_t>(alloc) != 0xffffffffffffffff)
					return from;
		...
		inline void* try_alloc_near(std::uintptr_t address) {
			...
			if (kMaxMemoryRange <= address && min_address < address - kMaxMemoryRange)
				min_address = address - kMaxMemoryRange;

			if (address + kMaxMemoryRange <= max_address)
				max_address = address + kMaxMemoryRange;
			...
			void* result = nullptr;
			{
				...
				while (min_address <= alloc) {
					...
					result = VirtualAlloc(reinterpret_cast<void*>(alloc), kMemoryBlockSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
					if (result != nullptr)
						break;
				}
			}
			...
			if (kMaxMemoryRange <= address)
				min_address = address - kMaxMemoryRange;

			//overflow check
			if (address < address + kMaxMemoryRange)
				max_address = address + kMaxMemoryRange;
