...
public:
    kthook_naked() : info(0, nullptr) {}

    kthook_naked(std::uintptr_t destination, bool force_enable = true) : info(destination, nullptr) {
        if (force_enable) {
            install();
        }
    }
...
private:
    const std::uint8_t* generate_relay_jump() {
        ...
        jump_gen->mov(rax, ptr[reinterpret_cast<std::uintptr_t>(&context.rax)]);
        jump_gen->jmp(ptr[rip]);
        jump_gen->db(reinterpret_cast<std::uintptr_t>(trampoline_gen->getCode()), 8);
        detail::flush_intruction_cache(jump_gen->getCode(), jump_gen->getSize());
        return jump_gen->getCode();
    }
